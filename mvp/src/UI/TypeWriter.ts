import { Color, EventEmitter, ExcaliburGraphicsContext, GameEvent, Graphic, ScreenElement, Vector } from "excalibur";
...
export class TypeWriter extends Graphic {
  ...
  constructor(config: TypeWriterConfig) {
    ...
    this.color = config.color ? config.color : Color.White;
...
export class TypeWriterUI extends ScreenElement {
  constructor(pos: Vector, dims: Vector, tw: TypeWriter) {
    super({
      pos,
      width: dims.x,
      height: dims.y,
